...
class Script
  ...
  def update(id, clean_name, sentence)
    @client.query("UPDATE hle_dev_test_dmitry_kachalov
                   SET clean_name = \"#{clean_name}\", sentence = \"#{sentence}\"
                   WHERE id = \"#{id}\";")
  end

  def select
    @client.query("SELECT * FROM hle_dev_test_dmitry_kachalov;")
  end
  ...
  def main
    select.each do |result|
      ...
      update(result['id'], clean_name, sentence)
    end
  end
  ...
  def slashes
    ...
    if tokens.length > 1
      ...
      tokens.each_with_index do |elem, index|
        @str += case index
                when 0 then elem
                when 1 then elem.include?('(') ? ' ' + elem : ' ' + elem.downcase
                else ' and ' + elem.downcase
                end
      end
    else
